...
class TFArgument(Argument):
    ...
    def random_tensor_value_range(dtype):
        ...
        if (
            ...
            "int64" in dtype.name
            or "int32" in dtype.name
            or "int16" in dtype.name
        ):
    ...
    def to_diff_code(self, var_name, low_precision=False) -> str:
        if self.type in [ArgType.LIST, ArgType.TUPLE]:
            ...
            if not low_precision:
                code += (
                    f"{var_name} = tf.cast({var_name}, tf.{self.dtype.name})\n"
                )
    ...
    def generate_arg_from_signature(signature):
        ...
        if label == "tf_object":
            if (
                ...
                return TFArgument(
                    None, ArgType.TF_TENSOR, minv, maxv, shape, dtype
                )
            if (
                signature["class_name"]
...
class TFAPI(API):
    def __init__(self, api_name, record=None) -> None:
        ...
        self.record = (
            TFDatabase.get_rand_record(api_name) if record is None else record
        )
    ...
    def to_code(
        ...
        if self.is_class:
            ...
            if inputs:
                arg_code += inputs.to_code(
                    input_name, low_precision=low_precision
                )
                res_code += (
                    f'{RES_KEY}["{res_name}"] = {cls_name}(*{input_name})\n'
                )
        else:
