...
class TorchLibrary(Library):
    ...
    @staticmethod
    def is_equal(x, y, diff_bound=1e-5, promoted=False):
        ...
        if x_type == ArgType.TORCH_TENSOR:
            ...
            if x.dtype != y.dtype:
                ...
                if not promoted:
                    ...
                    x = x.to(dtype=promoted_type)
                    y = y.to(dtype=promoted_type)
            ...
            for i in range(len(x)):
                if (
                    TorchLibrary.is_equal(x[i], y[i], diff_bound, promoted)
                    is False
                ):
                    return False
            return True
